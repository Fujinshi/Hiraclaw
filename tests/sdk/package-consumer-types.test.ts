...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@gitlawb/hiraclaw structure
  const pkgDir = join(tmpDir, 'node_modules', '@gitlawb', 'hiraclaw')
...
describe('package consumer types', () => {
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@gitlawb/hiraclaw/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@gitlawb/hiraclaw/sdk'`,
        ``,
