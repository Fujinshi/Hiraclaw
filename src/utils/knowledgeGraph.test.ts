...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  const configDir = mkdtempSync(join(tmpdir(), 'hiraclaw-knowledge-graph-'))
  ...
  it('deduplicates entities and updates attributes', () => {
    addGlobalEntity('tool', 'hiraclaw', { status: 'alpha' })
    addGlobalEntity('tool', 'hiraclaw', { status: 'beta', version: '0.6.0' })

    const graph = loadProjectGraph(cwd)
    const entities = Object.values(graph.entities).filter(e => e.name === 'hiraclaw')
