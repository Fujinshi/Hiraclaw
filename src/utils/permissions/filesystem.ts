...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.hiraclaw${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.hiraclaw${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.hiraclaw', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.hiraclaw', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.hiraclaw', 'skills')
