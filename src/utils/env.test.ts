...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'hiraclaw-env-test-'))
...
test('getGlobalClaudeFile: new install returns .hiraclaw.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.hiraclaw.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .hiraclaw.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.hiraclaw.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.hiraclaw.json'))
})
