...
beforeEach(() => {
  ...
  testConfigDir = mkdtempSync(join(tmpdir(), 'hiraclaw-provider-config-'))
...
describe('setActiveProviderProfile', () => {
  test('sets OPENAI_MODEL env var when switching to an openai-type provider', async () => {
    const configDir = mkdtempSync(join(tmpdir(), 'hiraclaw-provider-config-'))
  ...
  test('persists no-key openai-compatible profiles for restart fallback', async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(configDir, '.hiraclaw-profile.json'), 'utf8'),
      )

      expect(result?.id).toBe('ollama_prof')
      expect(existsSync(join(tempDir, '.hiraclaw-profile.json'))).toBe(false)
