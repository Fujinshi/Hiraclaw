    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.hiraclaw.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.hiraclaw.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.hiraclaw.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.hiraclaw.json is user-writable and cannot be trusted.
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'hiraclaw setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'hiraclaw auth login'.`,
    }
  }
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: hiraclaw auth login`,
  }
}
