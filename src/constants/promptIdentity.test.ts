...
;(globalThis as Record<string, unknown>).MACRO = {
  ...
  ISSUES_EXPLAINER: 'report the issue at https://github.com/Gitlawb/hiraclaw/issues',
  PACKAGE_URL: '@gitlawb/hiraclaw',
...
test('CLI identity prefixes describe Hiraclaw instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('Hiraclaw')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('Hiraclaw')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes Hiraclaw instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('Hiraclaw')
...
test('built-in agent prompts describe Hiraclaw instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Hiraclaw')
  ...
  expect(generalPrompt).toContain('Hiraclaw')
  ...
  expect(explorePrompt).toContain('Hiraclaw')
  ...
  expect(guidePrompt).toContain('Hiraclaw')
  expect(guidePrompt).toContain('You are the Hiraclaw guide agent.')
  expect(guidePrompt).toContain('**Hiraclaw** (the CLI tool)')
