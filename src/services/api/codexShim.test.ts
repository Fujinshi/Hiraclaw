...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'hiraclaw-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
    const output = webSearchToolTest.makeOutputFromCodexWebSearchResponse(
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'Hiraclaw repo',
                url: 'https://github.com/example/hiraclaw',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Hiraclaw is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/hiraclaw',
                  },
                ],
              },
            ],
      ...
      'Hiraclaw GitHub 2026',
      0.42,
    )

    expect(output.results).toEqual([
      'Hiraclaw is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/hiraclaw' },
                ],
              },
            ],
          },
