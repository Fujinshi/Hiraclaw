...
const { ChatController, HiraclawChatViewProvider, HiraclawChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const HIRACLAW_REPO_URL = 'https://github.com/Gitlawb/hiraclaw';
const HIRACLAW_SETUP_URL = 'https://github.com/Gitlawb/hiraclaw/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.hiraclaw-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('hiraclaw');
  const launchCommand = configured.get('launchCommand', 'hiraclaw');
  const terminalName = configured.get('terminalName', 'Hiraclaw');
...
async function launchHiraclaw(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Hiraclaw command not found: ${executable}. Install it with: npm install -g @gitlawb/hiraclaw`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(HIRACLAW_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(HIRACLAW_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Hiraclaw because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Hiraclaw wordmark">Open<span class="wordmark-accent">Claude</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Hiraclaw project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Hiraclaw commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class HiraclawControlCenterProvider {
  constructor() {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchHiraclaw();
          break;
        case 'launchRoot':
          await launchHiraclaw({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const provider = new HiraclawControlCenterProvider();
  ...
  const chatViewProvider = new HiraclawChatViewProvider(chatController);
  const chatPanelManager = new HiraclawChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) Hiraclaw';
  statusBarItem.tooltip = 'Open Hiraclaw Chat';
  statusBarItem.command = 'hiraclaw.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Hiraclaw';
        statusBarItem.tooltip = 'Hiraclaw is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Hiraclaw';
        statusBarItem.tooltip = 'Hiraclaw connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Hiraclaw';
        statusBarItem.tooltip = 'Open Hiraclaw Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('hiraclaw.start', async () => {
    await launchHiraclaw();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'hiraclaw.startInWorkspaceRoot',
    async () => {
      await launchHiraclaw({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('hiraclaw.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(HIRACLAW_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'hiraclaw.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(HIRACLAW_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'hiraclaw.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('hiraclaw.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.hiraclaw');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('hiraclaw.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('hiraclaw.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('hiraclaw.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('hiraclaw.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'hiraclaw.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('hiraclaw')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  HiraclawControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  HiraclawChatViewProvider,
  HiraclawChatPanelManager,
};
