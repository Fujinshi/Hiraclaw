...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'hiraclaw',
    launchCommand: 'hiraclaw --project-aware',
    terminalName: 'Hiraclaw',
    shimEnabled: false,
    workspaceFolder: '/workspace/hiraclaw/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/hiraclaw/very/long/path/example-project',
    launchCwdLabel: '/workspace/hiraclaw/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/hiraclaw/very/long/path/example-project/.hiraclaw-profile.json',
    workspaceProfilePath: '/workspace/hiraclaw/very/long/path/example-project/.hiraclaw-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Hiraclaw wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/hiraclaw\/very\/long\/path\/example-project"[^>]*>\/workspace\/hiraclaw\/very\/long\/path\/example-project<\//,
  );
});
...
test('HiraclawControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { HiraclawControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/hiraclaw/src/panels/control-center.js',
      workspacePath: '/workspace/hiraclaw',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/hiraclaw/src/panels',
      projectAwareCwdLabel: '/workspace/hiraclaw/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/hiraclaw',
      workspaceRootCwdLabel: '/workspace/hiraclaw',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/hiraclaw',
      activeFilePath: '/workspace/hiraclaw/src/panels/control-center.js',
      workspacePath: '/workspace/hiraclaw',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/hiraclaw',
      projectAwareCwdLabel: '/workspace/hiraclaw',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/hiraclaw/src/panels',
      launchCwdLabel: '/workspace/hiraclaw/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/hiraclaw\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/hiraclaw/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/hiraclaw',
      launchCwdLabel: '/workspace/hiraclaw',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/hiraclaw/);
  assert.match(html, /Same workspace-root target as Launch Hiraclaw because the relative command resolves from the workspace root · \/workspace\/hiraclaw/);
});
