# Hiraclaw on Android (Termux)

A complete guide to running Hiraclaw on Android using Termux + proot Ubuntu.
...
Hiraclaw requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
You should see:
```
✓ Built hiraclaw v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to Hiraclaw's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Hiraclaw repo.
