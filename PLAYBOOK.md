# Hiraclaw Local Agent Playbook

This playbook is a practical guide to run Hiraclaw with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Hiraclaw starts directly.
...
Or let Hiraclaw recommend the best local model for your goal:
...
```powershell
Get-Content .\.hiraclaw-profile.json
```
...
- Keep any legacy workspace `.hiraclaw-profile.json` local; new provider saves use the user config directory.
